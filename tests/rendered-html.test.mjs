...
test("server-renders the Eidolon official site", async () => {
  ...
  assert.match(html, /加一台/);
  assert.match(html, /按键说话器/);
...
test("server-renders the Eidolon One product page", async () => {
  ...
  assert.match(html, /THE ONE FAMILY/);
  ...
  assert.match(html, /从你想做的事开始/);
  assert.match(html, /One 里都有一份/);
  assert.match(html, /所有场景共用/);
  assert.match(html, /id="one-host"/);
  assert.match(html, /按键说话器/);
  assert.match(html, /桌面陪伴设备/);
...
test("IP ensemble page presents multi-device cast conversations truthfully", async () => {
  ...
  for (const mode of ["单独聊", "点名回应", "一起聊", "放手讨论", "随时打断", "安静陪伴"]) assert.ok(html.includes(mode));
  assert.match(html, /角色是角色/);
  assert.match(html, /按键说话器/);
  assert.match(html, /按下即停/);
  assert.match(html, /只有说话键在收音/);
...
test("smart home page presents the panel, host and phone roles truthfully", async () => {
  ...
  assert.match(html, /家里的几个时刻/);
  assert.match(html, /把家连进 One/);
  assert.match(html, /放心交给它/);
  assert.match(html, /家居中控面板/);
  assert.match(html, /快速识别/);
  assert.match(html, /不出声/);
