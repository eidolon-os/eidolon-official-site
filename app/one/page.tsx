...
export default function OnePage() {
  return (
    <main className="site-shell site-light one-v4-page">
      ...
      <section id="family" className="one-family-v4 section-pad"><div className="page-frame"><header className="section-heading split-heading"><div><p className="eyebrow">03 · THE PRODUCT MATRIX</p><h2>一台主机，四条产品线。<br />每条都对应真实的设备形态。</h2></div><p>每条产品线都围绕同一台主机：放在哪里、承担什么角色、为主机补上哪一种能力。</p></header><figure className="one-family-concept"><img src="/one-family-concept-v3.png" alt="Eidolon One 主机、可穿戴、房间终端、桌面设备与桥接模块产品家族" /><figcaption><span>ONE FAMILY</span><b>一台主机 · 四条外设产品线</b></figcaption></figure><div className="one-family-catalog">{family.map(item=><article key={item.name}><header><span>{item.n}</span><div><small>{item.code}</small><h3>{item.name}</h3><b>{item.role}</b></div><p>{item.promise}</p></header><div className={item.forms.length > 3 ? "has-four" : undefined}>{item.forms.map(([code,name,use])=><section key={name}><small>{code}</small><h4>{name}</h4><p>{use}</p></section>)}</div></article>)}</div></div></section>
